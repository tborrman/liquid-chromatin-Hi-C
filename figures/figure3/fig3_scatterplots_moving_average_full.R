...
pdf("scatterplot_get_moving_average_full.pdf", height=11, width=5)
par(mfrow=c(4,2), mar=c(5, 4, 0, 2) + 0.1)
...
plot(d$PC1, d$signal, col=ifelse(d$PC1 > 0, "red", "blue"),
     pch=20, xlab="PC1", ylab="DpnII-seq signal", cex=0.5, cex.lab=1.5)
rho <- sprintf("%.2f",round(cor(d$PC1, d$signal, method="spearman", use="complete.obs"), 3))
text(0.015, 600, bquote(rho == .(rho) ), cex=2)
lines(ma$w, ma$mu, pch=20, col="grey60", lwd=1.5)
...
plot(d$PC1, d$LOS_d, col=ifelse(d$PC1 > 0, "red", "blue"),
     pch=20, xlab="PC1", ylab="LOS DpnII (%)", cex=0.5, cex.lab=1.5)
rho <- sprintf("%.2f",round(cor(d$PC1, d$LOS_d, method="spearman", use="complete.obs"), 3))
text(0.015, 0.80, bquote(rho == .(rho) ), cex=2)
lines(ma$w, ma$mu, pch=20, col="grey60", lwd=1.5)
