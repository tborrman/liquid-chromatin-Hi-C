...
dev.off()
png("semi-log_cis_percent_A_bin.png", height=2000, width=2000, res=300)
plot(minutes, log2(A_cis), pch=20, col="red", ylab=bquote("log"[2]~"(Cis %)"),
     xlab = "Minutes after DpnII digestion", type="o",
...
png("semi-log_cis_percent_A_500bins.png", height=2000, width=2000, res=300)
plot(minutes, log2(A_cis), col="red", ylab=bquote("log"[2]~"(Cis %)"),
     xlab = "Minutes after DpnII digestion", type="l",lwd=0.5,
  ...
  if (!is.na(eigen[i, "PC1"])) {
    if(eigen[i, "PC1"] > 0) {
      A_cis <- c(MN[i, "cis"], hour1[i, "cis"],
                 hour2[i, "cis"], hour3[i, "cis"], hour4[i, "cis"],
                 hour8[i, "cis"], hour16[i, "cis"], hour24[i, "cis"])
      lines(minutes, log2(A_cis), col="red", lwd=0.5)
      counter = counter + 1
    }
  }
}
...
dev.off()
png("semi-log_cis_percent_B_bin.png", height=2000, width=2000, res=300)
plot(minutes, log2(B_cis), pch=20, col="blue", ylab=bquote("log"[2]~"(Cis %)"),
     xlab ="Minutes after DpnII digestion", type="o",
...
png("semi-log_cis_percent_B_500bins.png", height=2000, width=2000, res=300)
plot(minutes, log2(B_cis), col="blue", ylab=bquote("log"[2]~"(Cis %)"),
     xlab = "Minutes after DpnII digestion", type="l",lwd=0.5,
  ...
  if (!is.na(eigen[i, "PC1"])) {
    if(eigen[i, "PC1"] < 0) {
      B_cis <- c(MN[i, "cis"], hour1[i, "cis"],
                 hour2[i, "cis"], hour3[i, "cis"], hour4[i, "cis"],
                 hour8[i, "cis"], hour16[i, "cis"], hour24[i, "cis"])
      lines(minutes, log2(B_cis), col="blue", lwd=0.5)
      counter = counter + 1
    }
  }
}
