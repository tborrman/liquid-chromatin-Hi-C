...
parser=argparse.ArgumentParser(description='record CviQI sites for given chromosome FASTA in bed file format')
...
coords = Restriction.CviQI.search(seq_record.seq)

chrom = seq_record.id
OUT=open('CviQI_sites_' + chrom + '.bed', 'w')
for start in coords:
