...
size_df <- read.table("/cygwin64/home/Tyler/Research/digest/compartment_size/C-40000/eigen1_40kb.size.bedGraph", 
                      sep="\t", header=FALSE, col.names=c("chrom", "start", "end", "eigen", "size"))

df <- cbind(df, size_df["size"])
...
boundary_plot <- function(t, pc1, switch) {
  pdf(paste(switch,"/compartment_boundaries_", switch, "_half-life_LOS_PC1_merge_sizefix.pdf", sep=""), 
      height=6, width=6)
