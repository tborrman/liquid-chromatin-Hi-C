...
hp1_na <- df[c("half-life_LOS", "G1_Repli-seq", "S1_Repli-seq", "S2_Repli-seq", "S3_Repli-seq",
               "S4_Repli-seq", "G2_Repli-seq")]
colnames(hp1_na) <- c("hl", "G1", "S1", "S2", "S3", "S4", "G2")
hp1 <- na.omit(hp1_na)

# # Get upper quartile data 
upq_G1 <- hp1[hp1$G1 > quantile(hp1$G1)[4],]
upq_S1 <- hp1[hp1$S1 > quantile(hp1$S1)[4],]
upq_S2 <- hp1[hp1$S2 > quantile(hp1$S2)[4],]
upq_S3 <- hp1[hp1$S3 > quantile(hp1$S3)[4],]
upq_S4 <- hp1[hp1$S4 > quantile(hp1$S4)[4],]
upq_G2 <- hp1[hp1$G2 > quantile(hp1$G2)[4],]
...
#######################################################################################################################
pdf(paste("Repli-seq/half-life_Repli-seq_density_genome.pdf", sep=""),
          width=7, height=6)
      print(
      ggplot() + 
...
pdf(paste("Repli-seq/half-life_Repli-seq_cumulative_genome.pdf", sep=""),
    width=7, height=6)
print(
  ggplot() + 
