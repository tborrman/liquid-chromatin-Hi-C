...
hp1_na <- df[c("chrom", "half-life_LOS", "CBX1_R1", "CBX5", "LAD_K562","PCA_eigen1")]
colnames(hp1_na) <- c("chrom", "hl", "CBX1_R1", "CBX5", "LAD", "PCA_eigen1")
hp1 <- na.omit(hp1_na)
...
for (chrom in c(paste("chr", 1:22, sep=""), "chrX")) {
  ...
  upq_CBX1 <- hp1_chrom[hp1_chrom$CBX1_R1 > quantile(hp1_chrom$CBX1_R1)[4],]
  upq_CBX5 <- hp1_chrom[hp1_chrom$CBX5 > quantile(hp1_chrom$CBX5)[4],]
  upq_LAD <- hp1_chrom[hp1_chrom$LAD > quantile(hp1_chrom$LAD)[4],]
  ...
  #######################################################################################################################
  # B Compartments
  pdf(paste("chrom_B/", chrom, "_half-life_HP1_LAD_density_genome_B.pdf", sep=""),
            width=7, height=6)
        print(
        ggplot() + 
  ...
  pdf(paste("chrom_B/", chrom, "_half-life_HP1_LAD_cumulative_genome_B.pdf", sep=""),
      width=7, height=6)
  print(
    ggplot() + 
  ...
  ####################################################################################################################
  # # A Compmartments     
  # pdf(paste("chrom_A/", chrom, "_half-life_HP1_LAD_density_genome_A.pdf", sep=""),
  #     width=7, height=6)
  # print(
  #   ggplot() + 
