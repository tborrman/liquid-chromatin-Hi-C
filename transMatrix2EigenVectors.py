...
def main():
    ...
    scriptPath = "~/project/Research/ENCODE/cworld-dekker/scripts"
    ...
    # find nan cols
    verboseprint("finding nan cols and rows ... ",end="",file=sys.stderr)
    print(np.sum(np.isnan(matrix),0))
    print(len(np.sum(np.isnan(matrix),0)))   
    nan_cols = np.sum(np.isnan(matrix),0)==matrix.shape[0]
    print(nan_cols)
    print(len(nan_cols))
    valid_cols=np.invert(nan_cols)
    print(valid_cols)
    # find nan rows
    nan_rows = np.sum(np.isnan(matrix),1)==matrix.shape[1]
    valid_rows = np.invert(nan_rows)
    print(valid_rows)
    print(len(valid_rows))

    # remove nan rows and columns
    print(matrix[:,-1])
    matrix=matrix[-nan_rows,:][:,-nan_cols]
    verboseprint("done",file=sys.stderr)
    print(matrix.shape)
    print(matrix[:,-1])
    ...
    # since I already set up everything for doing PCA 
    # to get eigenvector with length equal to length of rows
    # going to transpose
    print(matrix.shape)
    matrix = matrix.T
    print(matrix.shape)
    ...
    pca_score,pca_v = calculate_eigen(matrix, 3)
    verboseprint("done",file=sys.stderr)
    ...
    # NOTE may have to do this for column too later
    egv1[~nan_rows]=eig1
    egv2[~nan_rows]=eig2
    egv3[~nan_rows]=eig3
    print(egv1)
    ...
    # Had to switch to header_cols here... 
    writeCompartmentFile(egv1,egv2,egv3,pca_score[0:3],geneDensity,header_cols,compartmentFile)
    ...
    print(eigenMultiplier, geneDensity)
    ...
    verboseprint("")
    # use header_cols again...
    writeCompartmentFile(egv1,egv2,egv3,pca_score[0:3],geneDensity,header_cols,compartmentFile)
    
    eig1BedGraphFile=inputMatrix_name+".eigen1.bedGraph"    
    # use header_cols again...
    writeBedGraphFile(egv1,pca_score[0],header_cols,inputMatrix_name,eig1BedGraphFile)
    
    verboseprint("drawing eigen plot (",inputMatrix_name,") ... ",end="",file=sys.stderr)
    eigenPlot = "./plotEigen.R"
    ...
    # print(len(header_cols))
    # print(len(valid_rows))
    # writeMatrix(header_rows[np.where(valid_cols)],header_cols[np.where(valid_rows)],corrMatrix,collapsed_corrMatrixFile)
    # verboseprint("done",file=sys.stderr)
    
    # print(np.c_[valid_rows])
    # quit()
...
def writeCompartmentFile(egv1,egv2,egv3,pca_score,geneDensity,header_rows,outfile):
    ...
    print(nan_geneDensity)
    print(len(header_rows))
    print(header_rows[1:10])
